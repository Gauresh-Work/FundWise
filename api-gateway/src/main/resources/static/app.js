...
const demoSeed = {
  ...
  navHistory: { 1: [{ id:1, schemeId:1, navDate:new Date().toISOString().slice(0,10), nav:148.35 }] }
};
...
const resources = {
  ...
  schemes: {
    ...
    fields: [field('schemeCode', 'Scheme code', 'text', true), field('name', 'Scheme name', 'text', true), select('schemeType', 'Scheme type', ['EQUITY', 'DEBT', 'HYBRID', 'INDEX', 'LIQUID'], true), field('expenseRatio', 'Expense ratio (%)', 'number', false, { min: 0, step: .01 }), field('currentNav', 'Current NAV', 'number', false, { min: 0, step: .0001 }), select('status', 'Status', ['ACTIVE', 'INACTIVE', 'CLOSED'], true)],
    columns: [nameColumn('name', 'schemeCode'), col('schemeType', 'Type'), valueColumn('currentNav', 'Current NAV', money), valueColumn('expenseRatio', 'Expense ratio', v => `${number(v)}%`), statusColumn()],
    special: 'nav'
  },
  folios: {
    singular: 'Folio', icon: '▤', description: 'Investor holdings mapped to mutual fund schemes',
    fields: [ref('investorId', 'Investor', 'investors', 'fullName'), ref('schemeId', 'Scheme', 'schemes', 'name'), field('folioNumber', 'Folio number', 'text', true), select('status', 'Status', ['ACTIVE', 'FROZEN', 'CLOSED'], true), field('currentUnits', 'Current units', 'number', true, { min: 0, step: .0001 })],
    columns: [nameColumn('folioNumber'), refColumn('investorId', 'Investor', 'investors', 'fullName'), refColumn('schemeId', 'Scheme', 'schemes', 'name'), valueColumn('currentUnits', 'Units', number), valueColumn('currentValue', 'Value', money), statusColumn()]
  },
  transactions: {
    singular: 'Transaction', icon: '⇄', description: 'Purchases, redemptions, SIPs and switches',
    fields: [ref('folioId', 'Folio', 'folios', 'folioNumber'), select('transactionType', 'Transaction type', ['PURCHASE', 'REDEMPTION', 'SIP', 'SWITCH'], true), field('transactionDate', 'Transaction date', 'date', true), field('amount', 'Amount', 'number', true, { min: 0, step: .01 }), field('nav', 'NAV', 'number', false, { min: 0, step: .0001 }), field('units', 'Units', 'number', false, { min: 0, step: .0001 }), ref('targetSchemeId', 'Target scheme (switch only)', 'schemes', 'name', false), select('status', 'Status', ['COMPLETED', 'PENDING', 'FAILED', 'CANCELLED'], true)],
  ...
  try {
    ...
    modalRoot.innerHTML = `<div class="modal-backdrop" data-close-modal><div class="modal wide statement-modal"><div class="modal-head"><div><h2>Folio statement</h2><p>${escapeHtml(folio.folioNumber || lookup('folios',folioId,'folioNumber'))}</p></div><button class="close-button" data-close>×</button></div><div class="modal-body"><div class="statement-sheet">
      ...
      <div class="detail-grid statement-summary">
        ...
        <div class="detail-item"><small>Current NAV</small><strong>${money(scheme.currentNav)}</strong></div>
        <div class="detail-item"><small>Investor</small><strong>${escapeHtml(investor.fullName || `#${folio.investorId || '—'}`)}</strong></div>
        <div class="detail-item"><small>Scheme</small><strong>${escapeHtml(scheme.name || `#${folio.schemeId || '—'}`)}</strong></div>
      </div>
