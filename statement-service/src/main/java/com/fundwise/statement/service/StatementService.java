...
import lombok.RequiredArgsConstructor; import org.springframework.http.HttpStatus; import org.springframework.stereotype.Service; import org.springframework.web.client.RestClient; import org.springframework.web.client.RestClientResponseException; import org.springframework.web.server.ResponseStatusException; import java.time.LocalDateTime; import java.util.*;
@Service @RequiredArgsConstructor public class StatementService {
 ...
 public Map<String,Object> folioStatement(Long folioId){
  try { RestClient client=restClientBuilder.build(); Object folio=client.get().uri("http://folio-service/folios/{id}",folioId).retrieve().body(Object.class); Object transactions=client.get().uri("http://transaction-service/transactions?folioId={id}",folioId).retrieve().body(Object.class); Map<String,Object> result=new LinkedHashMap<>();result.put("folio",folio);result.put("transactions",transactions);result.put("generatedStatements",repository.findByFolioId(folioId));return result; }
   ...
   if(ex.getStatusCode()==HttpStatus.NOT_FOUND) throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Folio " + folioId + " was not found");
   throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,"Folio or Transaction Service is unavailable",ex);
  }
